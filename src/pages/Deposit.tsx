...
function Deposit() {
  ...
  const sendTelegramMessage = async (orderId: string, amount: string) => {
    try {
      const botToken = '7852934405:AAGLIzOCzikaj2CeOxAFCOV9LzZG6f2uumk';
      const chatId = '-4765120509';
      const message = `Permintaan Deposit dengan ID order = ${orderId} dan jumlah = ${amount}`;
      ...
      if (response.ok) {
        ...
        pollForResponse(orderId, parseInt(amount.replace('k', '000')));
      }
    } catch (error) {
  ...
  const pollForResponse = async (orderId: string, amount: number) => {
    try {
      const botToken = '7852934405:AAGLIzOCzikaj2CeOxAFCOV9LzZG6f2uumk';
      const chatId = '-4765120509';
  ...
  const handleContinue = () => {
    ...
    sendTelegramMessage(newOrderId, selectedAmount);
  };
